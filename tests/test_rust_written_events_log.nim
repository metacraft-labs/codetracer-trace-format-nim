...
block a_frame_with_no_pledged_size_inflates:
  ...
  let got = inflateEventsLogChunk(UnpledgedFrame)
...
block an_empty_input_inflates_to_nothing:
  let got = inflateEventsLogChunk([])
  doAssert got.isOk
  doAssert got.get().len == 0

block a_damaged_frame_reports_an_error_rather_than_looping:
  ## The growth walk on an unpledged frame is bounded, so a frame that never
  ## decodes reports a named error instead of doubling its way to an OOM.  A
  ## truncated frame is the cheap way to produce one: flipping payload bytes
  ## is not, because this frame's literals are stored raw and no checksum is
  ## present, so a bit-flip decodes happily into different bytes.
  let truncated = @UnpledgedFrame[0 ..< 20]
  let got = inflateEventsLogChunk(truncated)
