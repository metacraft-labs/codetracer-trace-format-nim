...
proc test_meta_dat_strict_unknown_flag_rejection() {.raises: [].} =
  ...
  ##   * flags = bit 4 + bit 14 fails because bit 14 is unknown.
  ##     (Bits 5..13 are now allocated — bit 5 to
  ...
  block:
    # M23b: bit 10 (FlagHasValueStream) is now a KNOWN flag and must
    # round-trip cleanly with ``hasValueStream = true``.  A real M23b
    # bundle sets it alongside ``FlagHasCallStream`` and
    # ``FlagHasStepStream`` — assert all three round-trip together.
    let res = readMetaDat(craft(FlagHasValueStream))
    doAssert res.isOk,
      "bit 10 (FlagHasValueStream) must round-trip: " &
      (if res.isErr: res.error else: "ok")
    doAssert res.get().hasValueStream
    doAssert not res.get().hasStepStream
    doAssert not res.get().hasCallStream

    let resAll = readMetaDat(craft(
      FlagHasCallStream or FlagHasStepStream or FlagHasValueStream))
    doAssert resAll.isOk,
      "bit 8 + bit 9 + bit 10 (call+step+value streams) must round-trip: " &
      (if resAll.isErr: resAll.error else: "ok")
    doAssert resAll.get().hasCallStream
    doAssert resAll.get().hasStepStream
  ...
  block:
    # M23d: bit 12 (FlagHasInterningTables) is now a KNOWN flag and must
    # round-trip cleanly with ``hasInterningTables = true``.  A real M23d
    # bundle sets it alongside ``FlagHasCallStream`` / ``FlagHasStepStream``
    # / ``FlagHasValueStream`` / ``FlagHasIoEventStream`` — assert all five
    # round-trip together.
    let res = readMetaDat(craft(FlagHasInterningTables))
    doAssert res.isOk,
      "bit 12 (FlagHasInterningTables) must round-trip: " &
      (if res.isErr: res.error else: "ok")
    doAssert res.get().hasInterningTables
    doAssert not res.get().hasIoEventStream
    doAssert not res.get().hasValueStream
    doAssert not res.get().hasStepStream
    doAssert not res.get().hasCallStream

    let resAll5 = readMetaDat(craft(
      FlagHasCallStream or FlagHasStepStream or FlagHasValueStream or
      FlagHasIoEventStream or FlagHasInterningTables))
    doAssert resAll5.isOk,
      "bit 8 + bit 9 + bit 10 + bit 11 + bit 12 (all five) must round-trip: " &
      (if resAll5.isErr: resAll5.error else: "ok")
    doAssert resAll5.get().hasCallStream
    doAssert resAll5.get().hasStepStream
    doAssert resAll5.get().hasValueStream
    doAssert resAll5.get().hasIoEventStream
    doAssert resAll5.get().hasInterningTables

  block:
    # RS-M1 allocated bit 13 to FlagHasSpanStream, so it must now ROUND-TRIP
    # rather than reject — the same positive assertion bits 8..12 get above.
    let res = readMetaDat(craft(FlagHasSpanStream))
    doAssert res.isOk,
      "bit 13 (FlagHasSpanStream) must round-trip: " &
      (if res.isErr: res.error else: "ok")
    doAssert res.get().hasSpanStream
    doAssert not res.get().hasInterningTables
    doAssert not res.get().hasIoEventStream

    let resAll6 = readMetaDat(craft(
      FlagHasCallStream or FlagHasStepStream or FlagHasValueStream or
      FlagHasIoEventStream or FlagHasInterningTables or FlagHasSpanStream))
    doAssert resAll6.isOk,
      "bits 8..13 (all six stream bits) must round-trip: " &
      (if resAll6.isErr: resAll6.error else: "ok")
    doAssert resAll6.get().hasCallStream
    doAssert resAll6.get().hasStepStream
    doAssert resAll6.get().hasValueStream
    doAssert resAll6.get().hasIoEventStream
    doAssert resAll6.get().hasInterningTables
    doAssert resAll6.get().hasSpanStream

  block:
    # Bit 14 is allocated to FlagHasLineCountTable, so it must ROUND-TRIP
    # rather than reject — the same positive assertion every allocated bit
    # gets above.  It is checked ALONE: a header setting it together with
    # bit 4 declares the same paths.dat field under two record layouts, and
    # `writeMetaDat` refuses to produce one.
    let res = readMetaDat(craft(FlagHasLineCountTable))
    doAssert res.isOk,
      "bit 14 (FlagHasLineCountTable) must round-trip: " &
      (if res.isErr: res.error else: "ok")
    doAssert res.get().hasLineCountTable
    doAssert not res.get().hasSpanStream
    doAssert not res.get().hasColumnAwareSteps

  block:
    # Bit 15 (0x8000) is the last unallocated bit — retarget the
    # strict-rejection probe to it now that bit 14 is known.  When bit 15 is
    # allocated the flag word has to grow, and this probe has to be
    # rewritten against whatever that growth defines as unknown.
    const LastReservedBit: uint16 = 0x8000
    doAssert (LastReservedBit and KnownFlags) == 0,
      "the probe must name a bit this reader does NOT know, or it proves " &
      "nothing about the rejection contract"
    let res = readMetaDat(craft(FlagHasColumnAwareSteps or LastReservedBit))
    doAssert res.isErr,
