...
proc buildTrace(withLinehits: bool): seq[byte] =
  # An empty path selects the in-memory container: this test reads the
  # finished image straight out of `toBytes`, so it wants no file on disk and
  # is not entitled to a writable directory to put one in.
  var wr = initMultiStreamWriter("", "linehits_probe",
